...
public class Scanner implements Iterator<Token>, Iterable<Token>
{
    ...
    private String readNumber()
    {
        ...
        parsednumber.append(character);
        boolean isSeparatorParsed = false;
        while(true)
        {
            readChar();
            ...
            else if((character == '.' || character == ',') && isSeparatorParsed == false)
            {
                isSeparatorParsed = true;
                parsednumber.append(character);
                continue;
            }
            else if((character == '.' || character == ',') && isSeparatorParsed == true)
            {
                throw new RuntimeException("Number cannot have two separators");
            }
